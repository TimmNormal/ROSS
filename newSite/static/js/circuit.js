const SCROLL_TRIGGER = 253

const START_TOP_SLIDER = 75
const END_TOP_SLIDER = 175

var IS_CIRCUIT = false

start_banner.style.top = `${START_TOP_SLIDER}px`

function fade_slider_to(percent){
    let slider_top = percent * END_TOP_SLIDER / 100
    start_banner.style.top = `${START_TOP_SLIDER - slider_top}px`

    start_banner.style.opacity = `${1 - ((percent ) / 100)}`
    board_plate.style.opacity = `${((percent ) / 100)}`
}

window.addEventListener("scroll", (ev)=>{
    let scroll_percent = window.scrollY * 100 / SCROLL_TRIGGER

    scroll_percent = scroll_percent >= 100 ? 100 : scroll_percent
    ...
    scroll_percent = scroll_percent < 100 ? scroll_percent : 100

    fade_slider_to(scroll_percent)
...
function run(){
    ...
    // board_plate.scrollIntoView({block: "center", behavior: "smooth"})
...
function generate_board(){
    board_plate.innerHTML = `
    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 960 560" class="fade_circuit" style="enable-background:new 0 0 960 560; object-fit: cover;" xml:space="preserve" width="100vw" height="100vh" preserveAspectRatio="xMidYMid slice">
<g id="lines">
...
<polyline class="st0" points="651,330.5 654,330.5 657.5,334.3 657.5,340 662,343.5 670,343.5 692.5,326.5 724,326.5 742,314.5 931,314.5 944.5,325.5 960,325.5   "/>
...
<polyline class="st0" points="517.2,421.2 522.5,424.5 522.5,443 513.5,448.5 513.5,485 530.5,496.5 530.5,516 546.5,526.5 670,526.5 716,560   "/>
...
<polyline class="st0" points="635.5,421.4 642.5,424.5 642.5,439 648.1,442.5 698,442.5 707.5,434.5 722,434.5 730.3,442.5 842,442.5 864,458.5 960,458.5   "/>
...
board_parent.innerHTML += `
<img src="../static/images/headerLogo.png" alt="" class="fade_in_animation header__rCont__logo">
`
}